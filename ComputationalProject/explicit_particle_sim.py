...
class PIC_Explicit():
    def __init__(self, mx=128, dt=0.1, Lx=1.0, NPpCell=20):
        self.mx = mx
        self.dt = dt
        self.Lx = Lx
        self.dx = self.Lx / self.mx
        self.NPpCell = NPpCell
        self.Np = self.mx * self.NPpCell
        self.t = 0.0

        self.x = self.dx * np.arange(self.mx)
        ...
        self.E = np.zeros([3, self.mx])
        self.B = np.zeros([3, self.mx])
        self.rho = np.zeros(self.mx)
        ...
        vp1 = 1 + amp * np.sin(2 * np.pi / self.Lx * xp1)
        vp2 = -1 - amp * np.sin(2 * np.pi / self.Lx * xp1)
