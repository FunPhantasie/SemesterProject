...
# =============================================================
# Time grid (close to 1D instanton_Burgers style)
# =============================================================
class TimeGrid:
    def __init__(self, Nt, t_begin=-10.0, talpha=1.0):
        ...
        # exponential clustering toward final time te=0
        A = (te - tb) / (jnp.exp(-talpha * te) - jnp.exp(-talpha * tb))
        B = tb - A * jnp.exp(-talpha * tb)
        t = A * jnp.exp(-talpha * t) + B
        t = t.at[0].set(tb)
        t = t.at[-1].set(te)
        dt = jnp.roll(t, -1) - t
        self.t = t
...
# =============================================================
# 2D grids (pseudo‑spectral on [0,2π)^2)
# =============================================================
...
# =============================================================
# Instanton solver for 2D incompressible MHD in Elsässer variables (ω±)
# Structure mirrors a typical instanton_Burgers.py (1D) code
# =============================================================
class MHDInstanton2D(TimeGrid, RealSpaceGrid2D, FourierSpaceGrid2D):
    ...
    def nonlinearF(self, om_pF, om_mF):
        ...
        NpF =  (1j*kx)*(1j*ky)*F_xx + (1j*ky)*(1j*ky)*F_xy - (1j*kx)*(1j*kx)*F_yx - (1j*kx)*(1j*ky)*F_yy
        NmF =  (1j*kx)*(1j*ky)*F_xx + (1j*ky)*(1j*ky)*F_yx - (1j*kx)*(1j*kx)*F_xy - (1j*kx)*(1j*ky)*F_yy
    ...
    def RHS_adjointF(self, p_pF, p_mF, om_pF, om_mF):
        ...
        zpx, zpy = self.ifft2(zpxF), self.ifft2(zpyF)
        zmx, zmy = self.ifft2(zmxF), self.ifft2(zmyF)
        pp = self.ifft2(p_pF); pm = self.ifft2(p_mF)
