...
class FourierSpaceGrid2D:
    def __init__(self, Nx,Ny):
        ...
        kabs = jnp.sqrt(self.kx**2 + self.ky**2) * (Nx/(2*jnp.pi))
        kcut = (2.0/3.0)*(Nx/2.0)
        self.mask = (kabs <= kcut).astype(jnp.float32)


class MHDInstanton2D(TimeGrid, RealSpaceGrid2D, FourierSpaceGrid2D):
    def __init__(self,F, mu, a, out_dir,Nx=128,Ny=128,t_begin=-10.0 ,Nt=400, nu=1e-3, talpha=1.0 ):
        TimeGrid.__init__(self, Nt=Nt, t_begin=t_begin, talpha=talpha)
...
def run_CS(sim: MHDInstanton2D, p_F, u_F, dt):
    A = 0.
    epsilon = 1.e-8
    sigma = 1.
    delta_A = epsilon + 1
...
####***######
"""
mhd2d = MHDInstanton2D(Nx=Nx,Ny=Ny,t_begin=t_begin ,Nt=Nt, nu=nu, talpha=talpha, F=F, mu=mu, a=a, out_dir=out_dir)

u_F = jnp.zeros((mhd2d.Nt, mhd2d.kx.size), dtype=jnp.complex64)
p_F = jnp.zeros_like(u_F)

dF = -0.02
mhd2d.F = -0.001
while mhd2d.F < 100:
    p_F, u_F = run_CS(mhd2d, p_F, u_F, dt=mhd2d.dt)
    mhd2d.F += dF
