...
class htuck3d:
    ...
    def __init__(self, A=None, eps=1e-14):
        ...
        B12_3 = np.reshape(A, (N1 * N2, -1))  # B: Matricization, in slides called M
        # A 3d zu B12 n12xn3
        B3_12 = np.transpose(B12_3)

        U12, V12, r12 = svd_trunc(B12_3, eps)
        U3, V3, r3 = svd_trunc(B3_12, eps)



        G123 = np.tensordot(B12_3, U3, (1, 0))  # in slides denoted by fancy B
        G123 = np.tensordot(G123, U12, (0, 0)) #wtih pre step
        ...
        B3 = np.reshape(U3, (N3, -1))

        U3, V3, r3 = svd_trunc(B3, eps)

        U34r = np.reshape(U3, (N3,  r3))

        G34 = np.tensordot(U34r, U3, (0, 0))  # statt (0, 0)
    ...
    def full(self):
        ...
        print("U34.shape = ", U3.shape)
        print("should be: ", self.U3.shape)

        # U12, U34, G1234 --> A
        G123 = self.G123

        U34 = U3.reshape(N3, -1)
        A = np.tensordot(G123, U3, (0, 1))
...
mean_main = [0, 0, 0, 0]
covariance_main = [[1, 0, 0 ],
                   [0, 1, 0 ],
                   [0, 0, 1],
                   ]

mean_secondary = [1, -1, 1, -1]
covariance_secondary = [[0.5, 0, 0, ],
                        [0, 0.5, 0, ],
                        [0, 0, 0.5, ],
                        [0, 0, 0]]

shape = (N, N, N)  # Example shape
